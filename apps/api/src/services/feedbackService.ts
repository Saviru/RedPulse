...
export const feedbackService = {
  async getAll(filters?: FeedbackFilters): Promise<FeedbackItem[]> {
    ...
    if (filters?.userRole && filters?.userId) {
      if (filters.userRole === "user") {
        ...
        query.$or = [
          { assignToId: filters.userId },
          { assignToId: { $exists: false } },
          { assignToId: null },
        ];
      }
    }
  ...
  async update(id: string, input: UpdateFeedbackInput): Promise<FeedbackItem> {
    // Only $set the whitelisted fields. Everything else (id, userId, type,
  ...
  async updateComplaint(
    ...
    if (input.assignToId !== undefined) updates.assignToId = input.assignToId;
