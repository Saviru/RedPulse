...
export async function getMyCampaignRegistrations(
  ...
  try {
    ...
    const campaignIdsForFetch = registrations.map((r) => r.campaignId);
    const campaigns = await CampaignModel.find({ _id: { $in: campaignIdsForFetch } }).select("name");
    const campaignNamesMap = new Map(campaigns.map((c) => [c._id.toString(), c.name]));
    ...
    for (const registration of registrations) {
      ...
      const row = byCampaign.get(campaignId) ?? {
        campaignId,
        campaignName: campaignNamesMap.get(campaignId) ?? "Campaign",
        donor: null,
        volunteer: null,
      };
      ...
      if (registration.role === "VOLUNTEER") {
        const vr = registration.volunteerRegistration as any;
...
export async function submitTaskWork(req: Request, res: Response, next: NextFunction) {
  try {
    const authReq = req as AuthenticatedRequest;
    const { campaignId } = req.params;
    const userId = authReq.user!.id;

    const assignment = await VolunteerTaskAssignmentModel.findOne({ campaignId, userId });
    if (!assignment) {
      res.status(404).json({ message: "Task assignment not found" });
      return;
    }

    let workImages: string[] = assignment.workImages || [];
    if (Array.isArray(req.files)) {
      const newImages = (req.files as Express.Multer.File[]).map(
        f => `/uploads/task-submissions/${f.filename}`,
      );
      workImages = [...workImages, ...newImages];
    }

    assignment.workImages = workImages;
    await assignment.save();

    ok(res, assignment, "Work images uploaded successfully");
  } catch (error) {
    next(error);
  }
}
