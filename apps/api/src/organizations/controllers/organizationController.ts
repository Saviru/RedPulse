...
import { AuthRequest as AuthenticatedRequest } from "../../shared/middleware/auth.middleware";
...
async function buildCampaignOverview(campaign: any) {
  ...
  if (campaign.currentHospitalId) {
    hospital = await UserModel.findById(campaign.currentHospitalId);
    if (!hospital) {
      hospital = await OrganizationModel.findById(campaign.currentHospitalId);
    }
  }
  ...
  return {
    ...
    hospital: hospital
      ? {
          ...
          name: hospital.hospitalName || hospital.name || hospital.username,
...
export async function getHospitals(_req: Request, res: Response, next: NextFunction) {
  try {
    // Query users with role HOSPITAL who are verified
    const hospitals = await UserModel.find({ role: "HOSPITAL", isVerified: true }).sort({ username: 1 });
    
    // Map to the expected structure { id, name }
    const result = hospitals.map((h: any) => ({
      id: h._id.toString(),
      name: h.hospitalName || h.username || "Unknown Hospital"
    }));
...
export async function markVolunteerAttendance(
  ...
  try {
    ...
    // Credit points to user if ATTENDED and not already credited
    if (status === "ATTENDED") {
      const existingTx = await PointTransactionModel.findOne({
        username: (await UserModel.findById(registration.userId))?.username || "",
        targetId: campaign._id.toString(),
        type: "EARNED",
      });

      if (!existingTx) {
        await UserModel.findByIdAndUpdate(registration.userId, {
          $inc: { points: assignment.points || 0 },
        });

        await PointTransactionModel.create({
          username: (await UserModel.findById(registration.userId))?.username || "unknown",
          amount: assignment.points || 0,
          type: "EARNED",
          targetId: campaign._id.toString(),
          timestamp: new Date(),
        });
    ...
    await NotificationModel.create({
      ...
      message:
        status === "ATTENDED"
          ? `Your attendance for "${campaign.name}" was recorded as attended. ${assignment.points} points credited!`
...
export async function getDashboard(req: Request, res: Response, next: NextFunction) {
  try {
    const authReq = req as AuthenticatedRequest;
    const orgId = authReq.user!.id;

    const campaigns = await CampaignModel.find({ organizationId: orgId }).sort({ date: -1 });
    const campaignIds = campaigns.map((c) => c._id);
    ...
    for (const r of registrations) {
      if (r.role === "VOLUNTEER") {
        totalVolunteers++;
      } else if (r.role === "DONOR") {
        totalDonations++;
      }
    }

    const activeCamps = campaigns.filter(
      (c) => c.status === "PUBLISHED" || c.status === "PENDING_HOSPITAL",
    );
    // Sort upcoming camps by date ASC
    const upcomingCamps = campaigns
