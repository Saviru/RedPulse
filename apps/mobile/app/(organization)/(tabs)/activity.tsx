...
export default function OrganizationActivityScreen() {
  ...
  const fetchActivity = async () => {
    try {
      const data = await points.getOrganizationDonations();
      setDonations(data);
  ...
  const renderActivityCard = ({ item: donation }: { item: any }) => {
      ...
      <Card variant="elevated" style={styles.activityCard}>
        ...
        <View style={styles.detailsRow}>
          <View style={styles.detailItem}>
            <MaterialIcons name="volunteer-activism" size={16} color={colors.tint} />
            <Typo variant="caption" style={{ marginLeft: 4 }}>
              Donated {donation.amount} pts
            </Typo>
          </View>
        ...
        ListEmptyComponent={
          !isLoading ? (
            <View style={styles.emptyState}>
              <MaterialIcons name="volunteer-activism" size={64} color={colors.border} />
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            <View style={styles.modalHeader}>
              <Typo variant="h2">Verify Donor</Typo>
