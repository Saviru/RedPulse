...
export default function HospitalFeedbackManagementScreen() {
  ...
  const { feedbacks, addReply, updateReply, deleteReply, updateComplaint } = useFeedback();
  ...
  const handleRejectComplaint = (feedbackId: string) => {
    updateComplaint(feedbackId, {
      status: "rejected",
    });
  };

  const forThisHospital = feedbacks.filter(
    f => f.targetType === "hospital" && (f.targetId === HOSPITAL_ID || !f.assignToId || f.assignToId === HOSPITAL_ID),
  );

  const filteredFeedbacks = forThisHospital.filter(f =>
        ...
        {filteredFeedbacks.length === 0 ? (
          ...
          filteredFeedbacks.map(f => (
            <FeedbackCard
              ...
              onApproveComplaint={(id) => updateComplaint(id, { status: "in_progress" })}
            />
          ))
        )}
      </ScrollView>
